...
class UserService(object):
    ...
    def create_user(self) -> []:
        ...
        created = (datetime.utcnow() + timedelta(hours=9)).strftime("%Y-%m-%d %H:%M:%S")
        modified = (datetime.utcnow() + timedelta(hours=9)).strftime("%Y-%m-%d %H:%M:%S")
        return userid, email, password, username, phone, birth, address, job, interests, created, modified

    def create_users(self) -> []:
        rows = [self.create_user() for i in range(100)]
        columns = ['userid', 'email', 'password', 'username', 'phone', 'birth', 'address', 'job', 'interests', 'created', 'modified']
