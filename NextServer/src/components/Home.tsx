    ...
    return (
        ...
        <table className='list'>
            <thead>
                ...
                <th>생년월일</th><th>주소</th><th>직업</th><th>관심사항</th><th>카운트</th>
                </tr>
            </thead>
            <tbody>
            {list && list.map(({userid, email, password, username, phone, birth, address, job, interests, msg})=>(
                <tr key={userid}>
                    <td>{userid}</td><td>{email}</td><td>{password}</td><td>{username}</td>
                    <td>{phone}</td><td>{birth}</td><td>{address}</td><td>{msg}</td>
